...
export default function NewKampanjModal({ onClose, onCreate }: Props) {
  const today = new Date().toISOString().split("T")[0];
  const [name, setName] = useState("");
  const [start, setStart] = useState(today);
  const [end, setEnd] = useState(new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString().split("T")[0]);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) { setError("Ange ett kampanjnamn"); return; }
    setSaving(true);
    setError("");
    try {
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="px-6 pt-6 pb-5 border-b border-gray-100">
          <h2 className="text-lg font-semibold">Ny kampanj</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              placeholder="t.ex. Höstkampanj 2025"
